-- Drop the existing function first to avoid parameter name conflict error
DROP FUNCTION IF EXISTS create_initial_organization(text, text);

-- Improved onboarding function with duplicate check and unambiguous column references
...
BEGIN
  ...
  -- Use table alias "ur" to avoid ambiguity
  ...
  IF FOUND THEN
    -- If user already has an organization, return the existing info
    -- This prevents duplicate creation on double submission
  ...
  VALUES (v_new_org_id, p_store_name, upper(substring(p_store_name, 1, 3)) || floor(random() * 1000)::text)
