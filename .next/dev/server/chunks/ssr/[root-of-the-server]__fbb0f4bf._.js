...
async function Home() {
    ...
    const { data: userRole } = await supabase.from('user_roles').select('id').eq('user_id', user.id).maybeSingle();
    // if (!userRole) {
    //   // 역할 정보가 없으면 온보딩이 필요함
    //   redirect('/onboarding')
    // }
    // 온보딩이 완료된 경우 대시보드로 이동
    // 역할 정보가 없어도 대시보드로 이동하여 상태 확인 가능하게 함
...
//# sourceMappingURL=%5Broot-of-the-server%5D__fbb0f4bf._.js.map